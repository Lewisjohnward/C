...
int main (void){
    ...
    /* asign array of pointers to correct arr for iteration*/
    ptr[1] = arr1;
    ptr[2] = arr2;
    ptr[3] = arr3;
    ptr[4] = arr4;
    ...
    /* buffer contains what crane is moving */
    char crane[100];
    ...
    while(fgets(buf, 50, fp)){
        ...
        for(int i = 0; i < quantity; i++){
            /* move up destination NULL char by 1 postion */
            *(ptr[destination] + strlen(ptr[destination]) + 1) = '\0'; 
            /* put lowest char of source in destination underneath new null*/
            *(ptr[destination] + strlen(ptr[destination])) =  *(ptr[source] + strlen(ptr[source]) - (quantity - i)); 
        }
        /* replace moved char with null character / decrement source */
        *(ptr[source] + strlen(ptr[source]) - quantity) = '\0';
